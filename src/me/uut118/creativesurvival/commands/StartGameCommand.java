...
public class StartGameCommand implements CommandExecutor, TabCompleter {
	PluginMain plugin;
	public StartGameCommand(PluginMain plugin_) {
		plugin = plugin_;
	}
	@Override
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (gm.getGameRunning()) {
			sender.sendMessage(ChatColor.RED+"Error: Game already running.");
		} else {
			if (gm.beginGame())
				sender.sendMessage(ChatColor.GREEN+"Started the game.");
			else
				sender.sendMessage(ChatColor.RED+"Error: Too little players: Need at least 2.");
